...
export async function getSidebarSpaces(userId, accountId = null) {
  ...
  const nanolissEmails = [
    "nitesh@nanoliss.com",
    "hello@nanoliss.com",
    "refunds@nanoliss.com",
    "marketing@nanoliss.com",
    "support@nanoliss.com"
  ];

  const linearEmails = [
    "nitesh@linearventures.in",
    "info@linearventures.in",
    "accounts@linearventures.in",
    "export@linearventures.in",
    "suite@linearventures.in",
  ...
  spaces.push({ 
    name: "Nanoliss", 
    query: "(to:nanoliss.com OR from:nanoliss.com) -in:trash -in:draft",
    subSpaces: nanolissEmails.map(email => ({
      name: email,
  ...
  spaces.push({ 
    ...
    subSpaces: linearEmails.map(email => ({
      name: email,
      query: `(to:${email} OR from:${email}) -in:trash -in:draft`
    }))
  });
