...
export async function getSidebarSpaces(userId, accountId = null) {
  ...
  try {
    ...
    aliases = sendAsRes.data.sendAs || [];
  } catch(e) {
    console.error("Could not fetch sendAs aliases", e.message);
  }

  const mainEmail = aliases.find(a => a.isPrimary)?.sendAsEmail;

  // Real active aliases grouped by domain
  const linearAliases = aliases
    .filter(a => !a.isPrimary && (a.sendAsEmail.endsWith('@linearventures.in') || a.sendAsEmail.endsWith('@linearventures.com')))
    .map(a => a.sendAsEmail)
    .sort();

  const nanolissAliases = aliases
    .filter(a => !a.isPrimary && (a.sendAsEmail.endsWith('@nanoliss.in') || a.sendAsEmail.endsWith('@nanoliss.com')))
    .map(a => a.sendAsEmail)
    .sort();

  const isNanolissMain = mainEmail && (mainEmail.endsWith('@nanoliss.in') || mainEmail.endsWith('@nanoliss.com') || mainEmail === 'nanoliss.in@gmail.com');
  const isLinearMain = mainEmail && (mainEmail.endsWith('@linearventures.in') || mainEmail.endsWith('@linearventures.com'));
  ...
  if (isNanolissMain) {
    spaces.push({ 
      name: "Nanoliss", 
      query: "(to:nanoliss.in OR from:nanoliss.in OR to:nanoliss.com OR from:nanoliss.com) -in:trash -in:draft",
