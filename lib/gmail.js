...
export async function fetchEmails(userId, accountId = null, query = "") {
  try {
    ...
    // Fetch batch details
    const batchData = await fetchBatchMessages(gmail, messages.map(m => m.id));

    const enrichedEmails = batchData.map((data) => {
      const msg = data.data;
    ...
    const finalEmails = enrichedEmails.filter(e => e !== null);
