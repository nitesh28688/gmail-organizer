...
export async function getSidebarSpaces(userId, accountId = null) {
  if (accountId === "unified") {
    return [
      ...
      { name: "Services", query: "(receipt OR invoice OR order OR service) -bank -statement -payment -finance -in:trash -in:draft -in:spam" },
  ...
  // Group Organizer/* labels under a collapsible "Categories" section
  try {
    const gmail = await getGmailClient(userId, accountId);
    const labelsRes = await gmail.users.labels.list({ userId: "me" });
    const organizerLabels = (labelsRes.data.labels || [])
      .filter(l => l.type === "user" && l.name.startsWith("Organizer/"))
      .sort((a, b) => a.name.localeCompare(b.name));

    if (organizerLabels.length > 0) {
      spaces.push({
        name: "Categories",
        query: null,
        subSpaces: organizerLabels.map(label => ({
          name: label.name.replace("Organizer/", ""),
          query: `label:"${label.name}" -in:trash`,
          labelId: label.id,
        }))
      });
    }
  } catch (e) {
    console.error("Failed to load Organizer labels for spaces:", e);
  }
      ...
      messages.map(async (m) => {
        try {
          ...
          return {
            ...
            isImportant: msg.data.labelIds ? msg.data.labelIds.includes("IMPORTANT") : false,
          };
        } catch (err) {
...
export async function performAudit(userId, accountId) {
  ...
  const rules = [
    ...
    { name: "Organizer/Newsletters", query: "(newsletter OR digest OR \"weekly update\") -subject:invoice -subject:receipt -subject:order -subject:payment -subject:bill -subject:statement -subject:security -subject:OTP -in:trash -in:draft" }
  ];
